      ...
      {Array.from({ length: amount - 3 /* minus 3 images at the end, creates a gap */ }, (_, i) => {
        ...
        return (
          <Card
            ...
            active={hovered !== null}
            hovered={hovered === i}
            url={`${process.env.PUBLIC_URL || ''}/img${Math.floor(i % 10) + 1}.jpg`}
          />
        )
      })}
    </group>
...
function ActiveCard({ hovered, ...props }) {
  ...
  return (
    <Billboard {...props}>
      ...
      <Image ref={ref} transparent radius={0.3} position={[0, 1.5, 0]} scale={[3.5, 1.618 * 3.5, 0.2, 1]} url={`${process.env.PUBLIC_URL || ''}/img${Math.floor(hovered % 10) + 1}.jpg`} />
    </Billboard>
  )
}
